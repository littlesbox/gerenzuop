...
FeatureDir1 = '.\\FeatureForTrain\\NCDataFeature'
FeatureDir2 = '.\\FeatureForTrain\\SoilinfoFeature'
FeatureDir3 = '.\\FeatureForTrain\\YearBookFeature'
PAEsDataFilepath = '.\\PAEsData\\PAEsData_OK.csv'

#获取每个特征文件的路径，包括文件名
files_path = list_files(FeatureDir1) + list_files(FeatureDir2) + list_files(FeatureDir3)
file_csv_path = []
...
#读取PAEsData数据
PAEsData = pd.read_csv(PAEsDataFilepath, sep=',') 
...
traindata = []
for i in range(PAEsData.shape[0]):
    #获取样本信息
    SampleNum = i + 1
    SampleLocation3C = PAEsData.loc[i,'Location3C']
    SampleLocation4C = PAEsData.loc[i,'Location4C']
    SamplePlace = PAEsData.loc[i,'place']
    SampleTime = PAEsData.loc[i,'time']
    ...
    for item in FeatureInfo.index.tolist():
        ...
        if FeatureTimeResolution == 'Y' :   #如果当前特征的时间分辨率是 年
            ...
            if FeatureSumSence == 1: #如果具有可加意义
                colname = PAEsData.loc[i,FeatureSpatial] #取出当前样本的空间位置
            ...
            if FeatureSumSence == 1: #如果具有可加意义
                ...
                FeatureValuedict[item+'_td'] = np.std(FeatureValuelist,ddof=1)
...
traindata_df.insert(0, 'num', PAEsData['num'])
...
traindata_df4 = traindata_df3.copy()

#找出特征中的nan
nan_positions = traindata_df3.isna().stack()
nan_positions = nan_positions[nan_positions].index.tolist()
#找出PAEsConc中的nan
traindata_df3[traindata_df3["PAEsConc"] <= 0].index
...
# traindata_df3.to_excel('traindata.xlsx', index=False)
# traindata_df3.to_csv('traindata.csv', index=False)
