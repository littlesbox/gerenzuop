...
# 4. 划分数据集
X_train, X_test, y_train, y_test = train_test_split(X_augmented, y_augmented, test_size=split_rio,
                                                    random_state=split_rdseed)
...
# 7. 调优模型（可选）
param_grid = {
    # 'n_estimators': [(x+1)*5 for x in range(20)],  # 常见树的数量范围，50-200足够捕捉大多数数据集的特征 [(x+1)*5 for x in range(20)]
    ...
    # Param
    'n_estimators': [60,62,64],  # 常见树的数量范围，50-200足够捕捉大多数数据集的特征 [(x+1)*5 for x in range(20)]
    'max_depth': [16,17],  # 无限制和几个常见的深度限制，防止过拟合和欠拟合
    'min_samples_split': [2,3],  # 节点分裂所需的最小样本数，默认是2，适当增大以减少过拟合
    'min_samples_leaf': [1,2],  # 叶节点所需的最小样本数，默认是1，适当增大以减少过拟合
    'max_features': [0.6,0.7],  # 分裂时考虑的最大特征数，常见的选择是'auto'（所有特征）、'sqrt'（特征数的平方根）和'log2'（特征数的对数）
...
plt.savefig(os.path.join(SaveDirectory, 'RF_shap_summary_plot.png'),bbox_inches='tight')
plt.savefig(os.path.join(SaveDirectory, 'RF_shap_summary_plot.pdf'),bbox_inches='tight')
plt.close()
...
# plt.savefig(os.path.join(SaveDirectory, 'RF_shap_force_plot.pdf'))
# plt.close()
#
...
# plt.savefig(os.path.join(SaveDirectory, 'RF_shap_dependence_plot.pdf'))
# plt.close()
...
plt.savefig(os.path.join(SaveDirectory, 'RandomForestRegression.png'),bbox_inches='tight')
plt.savefig(os.path.join(SaveDirectory, 'RandomForestRegression.pdf'),bbox_inches='tight')
# # 添加图例
# ax.legend()
# 显示图形
plt.show()
...
# # 输出所有超参数组合及其 MSE 值
# print("每个超参数组合的 MSE 值：")
# for params, mean_mse, std_mse in zip(grid_search.cv_results_['params'], 
#                                      grid_search.cv_results_['mean_test_score'], 
#                                      grid_search.cv_results_['std_test_score']):
#     print(f"参数组合: {params}, 平均 MSE: {-mean_mse:.4f}, 标准差: {std_mse:.4f}")
...
plt.plot(indices, mse_values, marker='o', linestyle='-', color='#18a6de', label='MSE')
