...
param_grid = {
    ...
    # # Param
    'n_estimators': [200,300],  # 决策树的数量。更多的树可以提高模型的性能，但也会增加训练时间并可能导致过拟合。
    'learning_rate': [0.1],  # 每个树的贡献率。较低的学习率需要更多的树来达到相同的效果，较高的学习率则可能导致过拟合。
    'max_depth': [5,10],  # 每个决策树的最大深度。较深的树可以捕捉更多的细节，但也更容易过拟合。
    'min_samples_split': [2,4],  # 内部节点再划分所需的最小样本数。较大的值可以减少过拟合。
    'min_samples_leaf': [1,2],  # 叶节点所需的最小样本数。较大的值可以减少过拟合。
    'subsample': [0.6,0.7,0.8],  # 用于拟合每个决策树的样本比例。较小的值可以减少过拟合，但也可能增加方差
    'max_features': ['sqrt']  # 构建每个树时考虑的最大特征数。较小的值可以减少过拟合。
    # PCA Param
...
# plt.savefig(os.path.join(SaveDirectory, 'GBRT_shap_summary_plot.png'))
# plt.savefig(os.path.join(SaveDirectory, 'GBRT_shap_summary_plot.pdf'))
# plt.close()
...
plt.savefig(os.path.join(SaveDirectory, 'GBRT.png'))
# # 添加图例
# ax.legend()
# 显示图形
plt.show()
